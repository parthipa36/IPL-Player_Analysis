...
#setwd("C:\\Users\\rohan\\Desktop\\Praxis coureswork\\R programming\\ipl")
...
#df_master1 <- merge(df_master,res,by="Player_Id")
...
#df_master1 <- merge(df_master1,res,by=c("Player_Id","Season"))
df_masterOut <- merge(x = df_masterOut, y = res, by =c("Player_Id","Season"), all.x = TRUE)

#head(df_master1)
#head(subset(df_master1,df_master1$Player_Id==8))
...
#df_merged$Batting_points_zero_17 <- (df_merged$Consistency_bat_17/TBA)*df_merged$Runs_2017 + (df_merged$Fast_Scoring_ability_17/TBSR)*df_merged$Runs_2017

#df_merged[is.na(df_merged$Batting_points_zero_17),][,'Batting_points_zero_17'] <- 0


#df_merged$Bowling_points_zero_17 <- ((TBWA/PBWA) + (TBWER/PBWER)^2)*PWT

#df_merged[is.na(df_merged$Bowling_points_zero_17),][,'Bowling_points_zero_17'] <- 0
